...
import type { BuildTarget, BuildConfig, HotSwapConfig, CLIConfig } from "./index.js";
...
async function main(): Promise<void> {
    ...
    buildConfig: {
      targets:
        typeof flags["target"] === "string"
          ? (flags["target"].split(",") as BuildTarget[])
          : ["esm"],
  ...
  switch (command) {
    ...
    case "validate": {
      ...
      if (!schemaPath) {
        process.stderr.write(
          "validate requires a schema path argument\nUsage: obix validate <schema-path>\n"
        );
        process.exit(1);
      }
    ...
    case "hot-swap": {
      ...
      process.stdout.write(
        `Watching: ${watchPaths.join(", ")} (delay: ${delay}ms)\n`
      );
      ...
      for (const watchPath of watchPaths) {
        watch(watchPath, { recursive: true }, (_eventType, filename) => {
          ...
          debounceTimer = setTimeout(async () => {
            process.stdout.write(
              `[hot-swap] Change detected: ${filename ?? "unknown"}\n`
            );
            const result = await cli.build();
            if (result.success) {
              process.stdout.write(
                `[hot-swap] Rebuild complete in ${result.duration}ms\n`
              );
            } else {
    ...
    case "migrate": {
      ...
      if (!fromVersion || !toVersion) {
        process.stderr.write(
          "migrate requires two version arguments\nUsage: obix migrate <from-version> <to-version>\n"
        );
        process.exit(1);
      }
      await cli.migrate(fromVersion, toVersion);
      process.stdout.write(
        `Migration plan generated: ${fromVersion} -> ${toVersion}\n`
      );
      break;
    }
